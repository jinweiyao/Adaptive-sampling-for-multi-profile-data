...
out_se1<-output
for (i in 1:15){
  out_se1[2*i,]<-output[2*i,]/sqrt(100) #even line are standard errors
}

#final output in out_se1, take about 20 minutes for out_se1 to get ready
rown<-c('0.7','','1.0','','1.4','','2.1','','2.8','')
rownames(out_se1)<-c(rown,rown,rown)
colnames(out_se1)<-c('proposed','fpca','fpca001','fpca005','fpca01')

#For line plot plot 
poposed<-out_se1[seq(1,29,by = 2),1]
FCA<-out_se1[seq(1,29,by = 2),2]
FCA001<-out_se1[seq(1,29,by = 2),3]
FCA005<-out_se1[seq(1,29,by = 2),4]
FCA01<-out_se1[seq(1,29,by = 2),5]
